 ...
 *      3.1 属性，按是否使用static修饰，又分为静态属性 vs 非静态属性（实例变量）
 *      实例变量：我们创建了类的多个属性，每个对象都独立的拥有一套类中的非静态属性。当修改其中一个对象中的非静态属性时
 *              不会导致其他对象中同样的属性值的修改
 ...
 *   5.2 关于静态属性和静态方法的使用，大家都会从声明周期的角度去理解。
 *
 * 6. 开发中如何确定一个属性是否要声明为static的？
 *      > 属性是可以被多个对象共享，不会随着对象的不同而不同的。
 *      > 类中的变量也常常声明为static,
 *
 *
